      ...
      <div class="p-4 sm:p-6 md:p-10">
        <div class="flex flex-wrap items-start justify-between gap-4 pb-5 border-b border-slate-100">
          <div class="flex-1 min-w-0">
            ...
            <div class="flex flex-wrap items-center gap-3">
              ${ip.logoImage ? `<img src="${ip.logoImage}" alt="${ip.name} logo" class="h-10 sm:h-14 w-auto object-contain shrink-0" />` : ''}
              <h2 class="text-2xl sm:text-3xl md:text-4xl font-extrabold text-slate-900 font-display">${ip.name}</h2>
            </div>
            <p class="text-sm sm:text-base text-slate-600 font-medium mt-2">${ip.tagline}</p>
          </div>
          <button onclick="hIPlayApp.closeIPModal()" class="p-2.5 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-600 transition-colors shrink-0">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 my-6">
          <div class="lg:col-span-5 space-y-4">
            <div class="p-4 rounded-2xl bg-gradient-to-br from-slate-50 to-slate-100/70 border border-slate-200/60">
